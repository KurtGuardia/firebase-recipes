...
app.post('/recipes', async (request, response) => {
  ...
  const missingField =
    Utilities.validateRecipePostPut(newRecipe)

  if (missingField) {
    response
      .status(400)
      .send(
        `Recipe is not valid. Missing/invalid fields: ${missingField}`,
      )
    return
  }
...
app.get('/recipes', async (request, response) => {
  ...
  if (countDoc.exists) {
    ...
    if (countDocData) {
      recipeCount = countDocData.cound
    }
  }

  try {
    const firestoreResponse = await collectionRef.get()
    const fetchedRecipes = firestoreResponse.docs.map(
      (recipe) => {
        const id = recipe.id
...
app.put('/recipes/:id', async (request, response) => {
  ...
  if (missingFields) {
    response
      .status(400)
      .send(
        `Recipe is not valid. Missing/incalid fields: ${missingFields}`,
      )
    return
  }
